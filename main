...
def main():
    try:
        if os.name == "nt":
            ...
            pass
        banner()
        file = list(dict.fromkeys(open(input("[List] : ")).read().splitlines()))  # Membaca file daftar URL
        thread = int(input("[Thread] : "))  # Menentukan jumlah thread untuk pengecekan paralel
        pool = Pool(thread)
        pool.map(login, file)
        pool.close()
        pool.join()
